...
public class ContainedObject {
    ...
    public static final Codec<ContainedObject> CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(
                NbtCompound.CODEC.fieldOf("contained_mob").forGetter(ContainedObject::ToNbt)
        ).apply(instance, ContainedObject::new);
    });
    ...
    public EntityType<?> GetEntityType() {
        ...
        try {
            type = Registries.ENTITY_TYPE.get(Identifier.of(this.MobId));
    ...
    public <WriteTo> Entity ReleaseEntity(double X,double Y, double Z,World world,@Nullable PlayerEntity player) {
        if (world != null && !world.isClient) {
            ...
            entity.teleport((ServerWorld) world,X,Y,Z,null,0,0);
